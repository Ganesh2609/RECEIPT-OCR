...
from receipt_graph_dataset import ReceiptGraphDataset, FeatureExtractor
from layoutlm_transformer_gnn import LayoutImageBertGNN
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train LayoutImageBertGNN on receipt dataset')
    ...
    parser.add_argument('--vision_model', type=str, default='resnet18', 
                    choices=FeatureExtractor.get_supported_models(),
                    help='Vision model to use for image feature extraction')
    parser.add_argument('--roi_output_size', type=int, nargs=2, default=[7, 7], 
                    help='Output size for RoI pooling (height, width)')
    parser.add_argument('--use_roi_align', action='store_true', default=True,
                    help='Use RoI Align instead of RoI Pool (more precise but slower)')
    ...
    parser.add_argument('--bert_model', type=str, default='bert-base-uncased', 
                    help='BERT model to use as transformer encoder')
    parser.add_argument('--freeze_bert', action='store_true', 
                    help='Whether to freeze BERT parameters during training')
    parser.add_argument('--max_seq_length', type=int, default=512, 
                    help='Maximum sequence length for BERT (usually 512)')
    ...
    parser.add_argument('--output_dir', type=str, default='./results_bert_gnn', help='Output directory')
    ...
    _, _, feature_dim, _, _, _ = FeatureExtractor.create_model(args.vision_model, 'cpu')
    logger.info(f"Using vision model {args.vision_model} with feature dimension {feature_dim}")
    
    # Convert roi_output_size from list to tuple
    roi_output_size = tuple(args.roi_output_size)
    ...
    # Create model
    logger.info(f"Creating LayoutImageBertGNN model with {args.bert_model}...")
    model = LayoutImageBertGNN(
        hidden_channels=args.hidden_channels,
        out_channels=out_channels,
        image_embedding_dim=feature_dim,  # Use the feature dimension from the vision model
    ...
    logger.info(f"Using {args.gnn_layers} GNN layers with BERT transformer")
    if args.use_image_features:
        logger.info(f"Using image features from {args.vision_model} with dimension {feature_dim}")
    else:
    ...
    # Use different learning rates for BERT and other parameters if BERT is not frozen
    if not args.freeze_bert:
        # Parameters with different learning rates
        bert_params = list(model.bert_model.parameters())
        other_params = [p for n, p in model.named_parameters() if not n.startswith('bert_model.')]
        
        # Create parameter groups with different learning rates
        param_groups = [
            {'params': bert_params, 'lr': args.learning_rate * 0.1},  # Lower LR for BERT
            {'params': other_params, 'lr': args.learning_rate}
        ]
        optimizer = torch.optim.AdamW(param_groups, weight_decay=args.weight_decay)
        logger.info(f"Using different learning rates: {args.learning_rate * 0.1} for BERT, {args.learning_rate} for other parameters")
    else:
        # If BERT is frozen, use a single learning rate for all other parameters
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
        logger.info(f"BERT is frozen. Using learning rate {args.learning_rate} for all trainable parameters")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not args.freeze_bert:
            current_lr = optimizer.param_groups[1]['lr']  # Main learning rate (not BERT)
        else:
...
# Example usage:
# python train_bert_gnn.py --data_dir ./data_layoutlm --train_file train.txt --test_file test.txt --images_dir ./receipt_images --use_image_features --bert_model bert-base-uncased --max_seq_length 512--hidden_channels 256 --gnn_layers 2 --use_gat --gnn_heads 8 --dropout 0.3 --use_edge_features--learning_rate 3e-5 --scheduler cosine --class_weighting --output_dir ./results_bert_image_gnn


# python train_bert_gnn.py --data_dir ./data_layoutlm --train_file train.txt --test_file test.txt --images_dir ./receipt_images --vision_model vit --roi_output_size 7 7 --use_roi_align--use_image_features --bert_model bert-base-uncased --max_seq_length 512 --hidden_channels 256 --gnn_layers 2 --use_gat --gnn_heads 8 --dropout 0.3  --learning_rate 3e-5 --scheduler cosine --output_dir ./results_vit_bert_gnn